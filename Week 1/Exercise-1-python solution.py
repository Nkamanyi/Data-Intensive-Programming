...
cubeSum = sum([i**3 for i in myList])
...
# MAGIC %md
# MAGIC This codes simply count the occurence of each word in the phrase "sheena is a punk rocker she is a punk punk". The only difference is that they are implemented differently.
...
def cubeRoot(x: float, estimate: float = 1.0, epsilon: float = 0.0001) -> float:
    y = (2*estimate**3 + x) / (3*estimate**2)
    
    if abs(y - estimate) < epsilon:
        return y
    else:
        return cubeRoot(x, y, epsilon)
