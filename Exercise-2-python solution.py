...
weatherDataFrame: DataFrame = spark.read \
                                   ...
                                   .load('abfss://shared@tunics320f2023gen2.dfs.core.windows.net/exercises/ex2/rdu-weather-history.csv')
...
# DBTITLE 1,Schema of the original Data
weatherDataFrame1: DataFrame = spark.read \
                                    .format('csv') \
                                    .option('header','true') \
                                    .load('abfss://shared@tunics320f2023gen2.dfs.core.windows.net/exercises/ex2/rdu-weather-history.csv')

weatherDataFrame1.printSchema()

# COMMAND ----------

# MAGIC %md The column names for the csv file correspond with those of the DataFrame howerver, some of their datatypes are different at least for all the numeric columns. 
# MAGIC
...
minTemp: float = weatherDataFrame.agg(functions.min("temperaturemin")).first()[0]
maxTemp: float = weatherDataFrame.agg(functions.max("temperaturemax")).first()[0]
...
weatherDataFrameWithYear: DataFrame = weatherDataFrame.withColumn('year', functions.year('date'))
...
aggregatedDF: DataFrame = weatherDataFrameWithYear.groupBy('year').agg(functions.min('temperaturemin').alias('min_temperature'), functions.max('temperaturemax').alias('max_temperature'))

aggregatedDF = aggregatedDF.orderBy('year', ascending=False)
...
task6DF: DataFrame = weatherDataFrameWithYear.groupBy('year').agg(
    functions.min('temperaturemin').alias('min_temperature'),
    functions.max('temperaturemax').alias('max_temperature'),
    functions.count('*').alias('entries'),
    functions.round(functions.avg('avgwindspeed'), 2).alias('avg_windspeed'))
...
entries2018: int = task6DF.filter(task6DF.year == 2018).select('entries').first()[0]
wind2008: float = task6DF.filter(task6DF.year == 2008).select('avg_windspeed').first()[0]
...
yearWithMostDaysWithFog: int = weatherDataFrameWithYear.groupBy('year').agg(functions.sum(functions.when(weatherDataFrameWithYear.fog == 'Yes', 1).otherwise(0)).alias('fog')).orderBy('fog', ascending=False).select('year').first()[0]
