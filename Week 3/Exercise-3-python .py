...
retailerDataFrame: DataFrame = spark.read\
                                    .format('csv')\
                                    .option('delimiter',';')\
                                    .option('header','true')\
                                    .option('inferSchema','true')\
                                    .load('abfss://shared@tunics320f2023gen2.dfs.core.windows.net/exercises/ex3/sales_data_sample.csv')
...
best12DaysDF: DataFrame = retailerDataFrame\
                         .selectExpr('ORDERDATE','(QUANTITYORDERED * PRICEEACH) as total_sales')\
                         .groupBy(functions.col('ORDERDATE'))\
                         .agg(functions.sum('total_sales').alias('total_sales'))\
                         .orderBy(functions.col('total_sales').desc())\
                         .limit(12)
...
productDF: DataFrame = retailerDataFrame.filter(retailerDataFrame.YEAR_ID == 2003) \
    .withColumn("total_sales", functions.when(retailerDataFrame.MONTH_ID == 1, retailerDataFrame.QUANTITYORDERED * retailerDataFrame.PRICEEACH * 2).otherwise(retailerDataFrame.QUANTITYORDERED * retailerDataFrame.PRICEEACH)) \
    .groupBy(retailerDataFrame.PRODUCTCODE) \
    .agg(functions.sum('total_sales').alias('total_sales')) \
    .orderBy(functions.col('total_sales').desc()) \
    .limit(8)
...
sales2017Row: Row = salesDS.filter(salesDS.year == 2017).select("year","euros").first()
sales2017: Sales = sales2017Row
print(f"Sales for 2017 is {sales2017.euros}")

maximumSalesRow: Row = salesDS.orderBy("euros", ascending=False).select("year","euros").first()
maximumSales: Sales = maximumSalesRow
...
multiSalesDS: DataFrame = spark \
                          .createDataFrame(multiSalesList) \
                          .groupBy("year") \
                          .agg(functions.sum("euros").alias("euros"))

display(multiSalesDS)
multiSales2016: Sales = multiSalesDS.filter(multiSalesDS.year == 2016).select("year","euros").first()
print(f"Total sales for 2016 is {multiSales2016.euros}")

maximumMultiSales: Sales = multiSalesDS.orderBy("euros", ascending = False).first()
...
my_ex3_folder: str = 'stream_data'

targetFiles: str = f"abfss://students@tunics320f2023gen2.dfs.core.windows.net/ex3/{my_ex3_folder}/*"

staticSchema = retailerDataFrame.schema

retailerStreamingDF: DataFrame = spark.readStream \
                                      .format('csv') \
                                      .schema(staticSchema) \
                                      .option('delimiter',';') \
                                      .option('header', 'true') \
...
bestDaysDFStreaming: DataFrame = retailerStreamingDF \
        .selectExpr('ORDERDATE','(QUANTITYORDERED * PRICEEACH) as sales') \
        .groupBy(functions.col('ORDERDATE')) \
        .agg(functions.sum('sales').alias('total_sales')) \
...
# MAGIC
# MAGIC **It worked quite ok after receiving the tip from you.**
# MAGIC
# MAGIC What was the final output for the streaming data for you?
# MAGIC |ORDERDATE   |	total_sales
# MAGIC |---         | ---
# MAGIC |2004-11-24  |  115033.48000000001
# MAGIC |2003-11-14  |	109509.87999999999
# MAGIC |2003-11-12	 |  90218.58
# MAGIC |2003-12-02	 |87445.18000000001
# MAGIC |2004-10-16	 |86320.39000000001
# MAGIC |2003-11-06	 |84731.32
# MAGIC
